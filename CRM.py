...
# =========================
    ...
    # --- Bersihkan baris kosong / None ---
    df = df.dropna(subset=['Bulan','Jumlah Pelanggan'])
    df = df[df['Bulan'].astype(str).str.strip().str.lower() != 'none']
    
    # Kolom Jumlah Pelanggan jadi integer
    ...
    # --- Filter lagi sebelum tampil ---
    df_display = st.session_state.growth_data.copy()
    df_display = df_display.dropna(subset=['Bulan','Jumlah Pelanggan'])
    df_display = df_display[df_display['Bulan'].astype(str).str.strip().str.lower() != 'none']
    df_display = df_display.sort_values(by='Jumlah Pelanggan', ascending=False).reset_index(drop=True)

    # --- Tambah nomor urut sebagai kolom baru ---
    ...
    st.dataframe(df_display.style.apply(highlight_trend, axis=1).format({'Jumlah Pelanggan':'{:,.0f}'}))
