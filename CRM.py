...
st.set_page_config(page_title="CRM Textek", layout="wide")

st.title("CRM Textek - Streamlit Cloud Ready")

# --- File uploader ---
uploaded_file = st.file_uploader("Upload CRM Analyst.xlsx", type=["xlsx"])
...
if uploaded_file is not None:
    ...
    if current_sheet == "VIP BUYER":
        df = pd.read_excel(uploaded_file, sheet_name="VIP BUYER", engine='openpyxl')
    ...
    # =========================
        ...
        df = pd.read_excel(uploaded_file, sheet_name="Kategori Buyer", engine='openpyxl')
        df.columns = df.columns.str.strip()
    ...
    # =========================
        ...
        df_marketing = pd.read_excel(uploaded_file, sheet_name="Marketing Ads", engine='openpyxl')
        df_marketing.index = range(1, len(df_marketing)+1)

        st.subheader("Marketing Ads")
        fig, ax = plt.subplots()
        df_marketing.plot.pie(y='Jumlah', labels=df_marketing['Channel'], autopct='%1.1f%%', ax=ax)
        st.pyplot(fig)
    ...
    # =========================
        ...
        df = pd.read_excel(uploaded_file, sheet_name="Pertumbuhan Pelanggan", engine='openpyxl')
        df.columns = df.columns.str.strip()
        df = df.dropna(subset=['Bulan'])
        df['Jumlah Pelanggan'] = pd.to_numeric(df['Jumlah Pelanggan'], errors='coerce').fillna(0).astype(int)
        ...
        df_display.index = range(len(df_display))  # mulai dari 0 biar logika emoji benar
    ...
    # =========================
        ...
        df = pd.read_excel(uploaded_file, sheet_name="Produk Populer", engine='openpyxl')
        df.columns = df.columns.str.strip()
    ...
    # =========================
        ...
        df = pd.read_excel(uploaded_file, sheet_name="Produk Favorit Customer", engine='openpyxl')
        df.columns = df.columns.str.strip()
